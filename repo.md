...
│   ├── _cpp_sim.py         # CppSim — C++ SimWorld 桥接（601 行）
...
22 个已知问题（严重 #1–#5、中等 #6–#12、低 #13–#22）已于 2026-06 全部修复，913 测试通过。主要修复项：
...
| Map | Obs | env.step(ms) | C++(ms) | FPS | 瓶颈 |
|-----|-----|-------------|---------|-----|------|
| Sparse | 61 | **0.143** | 0.054 | **7,006** | Python 调用开销 |
| Maze | 1 | **0.289** | 0.223 | **3,457** | C++ LiDAR |
| Graph | 7 | **0.150** | 0.086 | **6,658** | C++ LiDAR |
| WFC | 5 | **0.298** | 0.225 | **3,351** | C++ LiDAR |
...
- [x] **P3: 动态障碍物位置同步** — `step_dynamic_obstacles` 中同步 `obs.vx/vy` 到 `obstacles_`；运动后重建 `lidar_grid_`
- [x] **P4: FMCW LiDAR 接入** — `SimWorld::fmcw_raycast_at` 直接使用 C++ obstacles_
- [x] **P5: 批量环境的对应优化** — BatchSimWorld 已验证
